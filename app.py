...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///assignment 3.db'
...
def query_grade(name):
	query_grade = db.session.query(Grades.username, Grades.assignment, Grades.grade, Grades.outof).filter(Grades.username == name)
	return query_grade
